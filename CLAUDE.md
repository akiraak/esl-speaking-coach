...
- **練習モードは 2 つ**（ヘッダのピルで切替。`PracticeMode`）: **会話モード**（トピックで雑談。誰も教えない）と **単語練習モード**（1 セッション 1 語を Chobi = 先生 / Naruko = 一緒に学ぶ生徒で練習）。モードで変わるのは system prompt・開始メッセージ・記憶ノートの有無・終わり方だけで、音声レイヤと出力形式は共通（仕様: `docs/specs/word-practice.md`）
