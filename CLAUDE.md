...
**このリポジトリは WSL(Linux) 上にある。iOS ネイティブのビルド・実機確認はここではできない。**

- ここでできること: Swift のコード生成・編集、設計、ドキュメント、プラン管理
- ビルド・実行・実機確認: **Mac 上の Xcode で行う**（リポジトリは git で共有する）
- したがって「ビルドが通ることを確認した」と書いてはいけない。未検証であることを明示して Mac 側に渡す
