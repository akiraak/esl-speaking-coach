...
- **練習モードは 3 つ**（ヘッダのピルで切替。`PracticeMode`）: **会話モード**（トピックで雑談。誰も教えない）、**単語練習モード**（1 セッション 1 語を Chobi = 先生 / Naruko = 一緒に学ぶ生徒で練習）、**単語クイズモード**（練習済みの語から未出題優先で 1 語を Chobi が出題し、出題後に自動終了。チャット欄には出題語を出さない）。モードで変わるのは system prompt・開始メッセージ・記憶ノートの有無・終わり方だけで、音声レイヤと出力形式は共通（仕様: `docs/specs/word-practice.md` / `docs/plans/archive/word-quiz-mode.md`）
