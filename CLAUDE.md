...
- **練習モードは 2 つ**（ヘッダのピルで切替。UI の練習モード `PracticeMode` = 会話 / 単語）: **会話モード**（トピックで雑談。誰も教えない）と**単語練習モード**（1 セッション 1 語を Chobi = 先生 / Naruko = 一緒に学ぶ生徒で練習）。単語モードのカードには練習の導線と同列で**単語クイズ**の開始ボタンがあり、練習済みの語から未出題優先で 1 語を Chobi が出題し、出題後に自動終了する（チャット欄には出題語を出さない）。クイズは UI モードではなく**セッション種別**（`SessionKind.quiz`。UI モードとセッション種別は別の型で、保存・プロンプト選択・セッション文言は `SessionKind` = 会話 / 単語 / クイズが持つ）。セッション種別で変わるのは system prompt・開始メッセージ・記憶ノートの有無・終わり方だけで、音声レイヤと出力形式は共通（仕様: `docs/specs/word-practice.md` / `docs/plans/archive/quiz-in-word-mode.md`）
...
- 会話履歴は端末内のみ。外部に送信するのは音声・会話系 API（Claude / OpenAI STT / Alibaba Qwen TTS / 切替時の Gemini ほか）へのリクエストと、単語帳（esl.chobi.me）の読み取り専用 API への検索クエリだけ
