...
| トピック提案 | Claude がトピック候補 3 件（日本語タイトル + フック 1 文）を生成してカードで提示。再生成・自作トピック・フリートークにも対応。ジャンル × 話し方 × 難易度はアプリ側でサンプリングして割り当て、候補が単調にならないようにしている |
| セッション後フィードバック | セッション終了時に会話全文を `claude-opus-5` で評価し、日本語のフィードバックカードをタイムラインへ投稿する |
...
| フィードバック | `claude-opus-5` |
| トピック生成 / 記憶更新 | `claude-sonnet-5` |
| 翻訳 | `claude-haiku-4-5` |
