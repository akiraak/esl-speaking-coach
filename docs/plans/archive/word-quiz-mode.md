...
| 1 セッションの構成 | 選んだ**最大 5 語（`quizWordCount`）を 1 セッションで順に出題**。全語出し終えたら Chobi が短く締めて**自動終了**する | 単語モードの「1 語をじっくり」とは役割が違う（教える vs 思い出させる）。5 語で 1 周 10 分程度を想定し、テストのように「終わりがある」ほうがクイズとして自然。もう 1 周したければカードからすぐ次のクイズを始められる（未出題優先により別の語が出る） |
...
| 終わり方 | **自動終了**（`endsOnGoodbye` = true）。全語を出し終えたら Chobi が短い締め（ねぎらい + 全語のふりかえり 1 行）を言い、制御行 `[end]` を出してセッションを終える。学習者が明確にやめたがったときも会話モードと同じ規定で `[end]`。終了ボタンでの途中終了も従来どおり可能 | クイズは 5 語で 1 周という明確なゴールがあるので、会話・単語モードと違い「終わり」をプロンプトが判定できる。既存の `[end]` 検知 → `.sessionEndDetected` → フィードバックの経路（`TurnBasedVoiceSession.handleTurnFinished`）がそのまま使える |
...
- `PracticeMode` に `case quiz` を追加し、全プロパティを埋める:
  ...
  `feedbackTopicLabel` = `Quiz words` / `endsOnGoodbye` = true（全語出し終えたら
  プロンプトが `[end]` を出して自動終了。既存の goodbye 終了経路をそのまま使う）
...
- `QuizCoachSystemPrompt.swift` を新規作成。構成は `WordCoachSystemPrompt` と同じ骨格
  ...
  3. 答えられたら短く確認し、**自分の文で使わせてから**次の語へ。間違い・度忘れはヒント → 正解を
     短く教えて使わせて次へ（引っ張らない）
  4. **全語を出し終えたら自動終了**: 短い締め（ねぎらい + 出題した語のふりかえり 1 行）を言い、
     最終行に `[end]` だけを出す（`CoachSystemPrompt` の Session flow の文面を踏襲）。
     学習者が明確にやめたがったときも同じ規定で `[end]`。途中の言い淀み・沈黙では出さない
- 出力形式（`Chobi: ` / `Naruko: ` タグ・1 ターン 1 質問・英語のみ・STT 誤認識の扱い）は
  word プロンプトの文面を踏襲する
- トークン数を数え、1024 未満なら例示を足して満たす（実装時に確認してプランに追記）
  → **実装時に count_tokens API で実測: 約 2,700 トークン**（1024 を満たす）
...
- シミュレータ E2E: `-practice-mode quiz -start-quiz -send-text ... -end-session` で
  ...
  (3) 練習済みが 1 語だけの状態で全問答えると AI が締めて**自動終了** → フィードバックへ進む
  （5 語をテキストで答え切るのは長いので、母集団を 1 語にして終了経路を確認する）
  (4) 途中の終了ボタンでも終われて `Quiz words:` ラベルでフィードバック
