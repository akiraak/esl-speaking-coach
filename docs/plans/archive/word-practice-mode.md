...
ドキュメント（2026-07-28 に実施。親項目を閉じる時点でまとめて書いた）:

- `docs/specs/word-practice.md` を新規作成（モードの仕様・会話モードとの差分早見表・
  system prompt の確定版）
- `docs/specs/screen-layout.md` にモードピル・単語カード・文言の出し分け・受け入れ条件を追記
- `docs/specs/conversation-design.md` に「このドキュメントは会話モードの設計」の但し書きと、
  キャラの立ち位置・`[end]` 不使用の差分を追記
- `CLAUDE.md`「プロダクト概要」に練習モードが 2 つあることを追記
...
- [x] ヘッダのモードピルで会話 ⇄ 単語を切り替えられ、アプリ再起動をまたいで保持される
- [x] セッション中はモードを切り替えられない
- [x] 単語モードのカードから単語を入力してセッションが始まり、Chobi の導入ターンが読み上げられる
- [x] Chobi が先生として教え、Naruko が学習者と同じ立場で先に使ってみる進行になる
- [x] 学習者が練習語を使うと、別の文脈でもう一度使うよう促される
- [x] 練習語に関係しない誤りは会話中に指摘されない
- [x] goodbye と言ってもセッションは終わらず、別の文脈で練習が続く
- [x] 「この単語を終了」ボタンでのみ終了し、フィードバックカードが出る。記憶ノートは更新されない
- [x] 練習した単語がセッション履歴に保存され、管理画面から見える
- [x] 会話モードに戻すと、従来どおりトピックカード（持ち越し込み）で会話が始まる
- [x] 既存ストアを持つ端末でマイグレーション後も過去の履歴が読める
