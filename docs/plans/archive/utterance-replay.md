...
> **完了（2026-08-01）**: Phase 1〜3 実装済み・単体テスト 12 件 + シミュレータ E2E
> （保存 → ファイル再生 / キャッシュ削除 → 再生成 / 新セッション開始で前セッション分の削除）
> + 実機確認済み。E2E 用の起動引数 `-replay-latest`（最後の AI 吹き出しをタップした扱い）を追加してある。
