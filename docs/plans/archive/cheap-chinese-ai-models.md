...
## Phase 5 具体化（2026-07-31 決定）

### 方針: 実データ駆動に切り替える

Phase 3 は system prompt・リクエスト形こそ本番同一だったが、入力データは「3 ターン程度の短い履歴 + 合成発話」で、実運用（15 分 / 30 ターン・STT を通った崩れのある学習者発話・履歴が数万トークン積み上がった状態）と乖離していた。Phase 5 は**実機の実セッションを fixture 化**して評価する。判定方法（機械チェック + opus-5 ブラインド A/B・順序入替 2 回・不一致 tie）は Phase 3 と同一とし、変えるのは入力データと規模のみ。

- **Step 0（実装）**: DEBUG 限定のセッションエクスポート導線。管理画面「会話」に書き出しボタンを置き、全セッション（発話・翻訳・調査ログ・フィードバック・現在の記憶ノート）を JSON 化して共有シート（AirDrop）で Mac へ渡す。セッション当時の記憶ノートは保存されていないため、[Memory:] 行の再現は現在値ベースの近似になる点は制約として記録する
- **Step 1**: 実機からエクスポートして fixture 化。API 履歴の再構築（連続 AI 発話を `Chobi: ` / `Naruko: ` タグ付き台本 1 メッセージへ再直列化、開始の制御メッセージ合成）は本番 `ChatRoomStore.rebuildHistory` と同じ規則を評価スクリプト側に実装する
- **Step 2（単発ターン評価）**: ケース 4 → 10（continue 変形 4 / goodbye 2 / japanese 2 / newtopic 2）。土台の履歴は実セッション由来とし、発生頻度の低い japanese 等のみ合成で補う。各ケース × 各モデル **5 生成（sonnet-5 も同数）** → 機械チェック 50 サンプル/モデルで違反「率」を出す。長い実履歴（20 ターン超）上での生成をケースに含める
- **Step 3（マルチターン再生評価）**: 実セッションの学習者発話を固定し、AI ターンだけを候補モデルで先頭から順に再生成して 20〜30 ターン通す（候補モデル自身の出力が履歴に積まれる状態を再現）。形式維持率・キャラ / 話題の維持を機械チェック + 窓単位の judge で見る
- **Step 4**: 単語練習・クイズの system prompt でも生成 + 機械チェック（クイズの自動終了・出題語をチャットに出さない等の規約）。フィードバックは transcript 4 本（実セッション + 実データ文体の補完）× 各モデル 3 生成で schema 適合と品質を判定
- **Step 5（判定）**: opus-5 ブラインド A/B 約 170 回（会話 10 ケース × 3 ペア × 順序 2 × 候補 2 + フィードバック 4 × 3 × 2 × 2）。判定コスト目安 $10 前後（$5/$25 per MTok）。候補側の生成は Qwen = SG 無料枠内 / deepseek = OpenRouter 残高で賄う

### 合否基準（実行前に固定）

- **会話**（qwen3.7-plus / qwen3.7-flash、それぞれ独立に判定）: (a) 致命的形式違反率（[end] 欠落・誤発火、最終行タグ無し、3 発話、CJK 混入）が sonnet-5 の実測違反率以下、かつ (b) judge 通算勝率（勝 1 / 分 0.5）≥ 45% で continue/goodbye 群に絞っても一貫負けが消えていること、かつ (c) マルチターン再生で形式維持が sonnet-5 同等。→ 全て満たせば置き換え候補へ昇格、欠ければ会話は sonnet-5 維持で確定（再評価しない）
- **フィードバック**: schema 適合 100%・捏造 / 誤指摘ゼロ・judge 勝率 ≥ 45%。qwen3.7-plus 通過で実装タスクへ。qwen 落ち & deepseek 通過の場合は OpenRouter 依存のトレードオフを再評価してから繰り上げ判断
- 東京リージョンのレイテンシ実測は引き続きスコープ外（採用決定後の実装検証で必要なら実施）

## Phase 5 実施記録（2026-07-31）: 生成品質の厳密チェック → **全経路で置き換え見送り**

**Phase 5 完了・調査タスク全体の結論確定**。実機の実セッション 62 本（会話 32 / 単語 23 / クイズ 7、STT 誤認識込みの本物の学習者発話）を fixture 化し、単発ターン 225 生成・マルチターン再生 75 生成・フィードバック 36 生成を機械チェック + opus-5 ブラインド A/B 102 ペア（204 呼び出し、判定コスト $10 前後）で評価した。**会話・フィードバックとも全合否基準に不合格。中国系 LLM への置き換えは行わない。**

### 実施方法

- Step 0 の DEBUG エクスポート導線（管理画面 → JSON → AirDrop）で実機から全セッションを取得。API 履歴の再構築（台本タグ再直列化・開始制御メッセージ合成）は本番 `rebuildHistory` / `SessionOpeningMessage` と同一規則をスクリプトに実装し、全 62 セッションで role 交互違反ゼロを確認
- 会話 11 ケース（continue 変形 5 / goodbye 2 / japanese 2 / 開幕 2。japanese は実データに本物あり）× 3 モデル × 5 生成。単語・クイズ 4 ケース × 同。フィードバックは実 transcript 4 本 × 3 モデル × 3 生成。再生は 2 セッション（10 / 15 学習者ターン）で学習者発話を固定し AI ターンのみ順次再生成
- リクエスト形は本番同一（会話: effort low / max_tokens 1024 / stream / cache_control、Qwen は `thinking: {type:"disabled"}` 付き。フィードバック: effort high / json_schema）

### 結果 1: 機械チェック（会話ケース 55 生成 / モデル。致命的 = タグ無し・[end] 違反・CJK 混入・発話数超過）

| モデル | clean | 致命的違反 | 特徴的な違反 |
| --- | --- | --- | --- |
| sonnet-5（基準） | 50/55 (91%) | **2/55 (4%)** | 空行・稀なタグ無し行 |
| qwen3.7-plus | 46/55 (84%) | 8/55 (15%) | **開幕系で暴走**（7 行生成 + アプリ制御行 `[New topic: 好きな食べ物]` を日本語で捏造）、continue-early で 3 発話癖 5/5、quiz の [end] を発話行末尾に付けてパーサ破壊 |
| qwen3.7-flash | 34/55 (62%) | 7/55 (13%) | japanese 対応崩れ、quiz-end の [end] 欠落 5/5 |

### 結果 2: マルチターン再生（自己出力が履歴に積まれる状態。致命的違反率）

| モデル | replay-ai (10T) | replay-morning (15T) |
| --- | --- | --- |
| sonnet-5 | **0/10** | **0/15** |
| qwen3.7-plus | 9 ターン中 8 で 3 発話化（自己強化ドリフトで崩壊） | 0/15 |
| qwen3.7-flash | 0/10 | 14 ターン中 14 で 3 発話化（崩壊） |

- 両候補とも 2 セッション中 1 つで「一度 3 発話を出すと自身の履歴がそれを規範化し全ターン 3 発話に固定される」崩壊を起こした。sonnet-5 はゼロ。単発ターン評価では原理的に検出できない失敗モードで、マルチターン再生を追加した価値がここに出た

### 結果 3: opus-5 ブラインド A/B（対 sonnet-5。勝率 = (勝 + 分×0.5) / 総数）

| 評価 | qwen3.7-plus | qwen3.7-flash | deepseek-v4-flash |
| --- | --- | --- | --- |
| 会話 単発（33 ペア） | 11勝4分18敗 **39%** | 5勝5分23敗 **23%** | −（対象外） |
| 会話 再生窓（6 ペア） | 0勝1分5敗 **8%** | 0勝0分6敗 **0%** | − |
| フィードバック（12 ペア） | 1勝2分9敗 **17%** | − | 3勝0分9敗 **25%** |

- qwen3.7-plus が一貫して勝ったのは continue-long / continue-question のみ。開幕系（newtopic / learnerfirst）と japanese 系で一貫して負け、判定理由は「制御行の捏造」「喋りすぎで学習者の発話機会を圧迫」「記憶ノートを活用しない」
- フィードバックの schema 適合は 3 モデルとも 12/12。品質は sonnet-5 が明確勝ち。判定理由は「note の解説が実際の誤りと食い違う」「try_phrases の日本語訳が不正確」「summary の具体性・学習者の引用の欠如」

### 判断（Phase 4 の暫定判断を更新して確定）

| 経路 | 判断 | 根拠 |
| --- | --- | --- |
| 会話ターン | **置き換えない（確定・再評価しない）** | 合否基準 (a)(b)(c) すべて不合格。特にマルチターン崩壊は致命的 |
| フィードバック | **置き換えない**（Phase 4 の「置き換える方向」を撤回） | schema は問題ないが品質で明確負け（qwen 17% / deepseek 25%）。月 $1.5 の節約で品質を落とす理由がない |
| トピック / 記憶ノート / 翻訳 | 置き換えない（Phase 4 のまま） | 節約がほぼゼロ |
| TTS / STT | **別タスクとして継続**（`TODO.md` 済） | LLM と独立に約 1/3 の単価。こちらが本命の節約 |

### 教訓

- **Phase 3（合成 transcript 1 本のスモークテスト）とフィードバックの結論が逆転した**（Phase 3: qwen 惜敗 / deepseek 明確勝ち → Phase 5 実データ: 両方明確負け）。合成データは学習者発話が理想化されすぎており、STT ノイズ・実際の誤り分布・実セッションの長さの下での品質差を反映しない。今後モデル比較をする場合は最初から実データ + 複数生成 + マルチターン再生で行う
- Step 0 のエクスポート導線と fixture 化スクリプト・評価ハーネス（scratchpad の phase5 一式）は再利用可能。新モデルが出たときは同じハーネスで再評価できる

### 付録: 不採用理由の短いまとめ（全候補）

**会話ターン**

- **qwen3.7-plus** — 速さと料金（1/9）は文句なしだったが、会話の開始ターンでアプリの制御行を捏造して 7 行も一人で喋る暴走があり、長いセッションでは「1 ターン 2 発話まで」のルールが自分の過去出力につられて全ターン 3 発話に崩れた。opus-5 の目隠し比較でも sonnet に勝率 39% と及ばず不採用
- **qwen3.7-flash** — 最安クラス（1/60）で短い応答は速いものの、形式違反が全体の 4 割近くあり、クイズの終了合図 `[end]` を 5 回中 5 回出し忘れ。品質勝率も 23% で、安さでは埋まらない差だった

**フィードバック生成**

- **qwen3.7-plus** — JSON の形式は毎回完璧だったが中身が弱い: 指摘の解説文が実際の誤りと食い違う、おすすめ表現の日本語訳が不正確、総評が一般論で学習者の発話を引用しない。勝率 17% で不採用
- **deepseek-v4-flash** — 合成データの Phase 3 では sonnet に勝っていたのに、実データ（音声認識ノイズ込み・4 セッション）では「誤りでないものまで訂正する」「言い換えが不自然」が出て勝率 25% に逆転負け。実データで検証し直した価値が一番出た項目

**早い段階で落ちたもの**

- **GLM-4.7 / 5.2** — Phase 3 で判定 0 勝の最下位。台本の最終行にタグを付けない癖（アプリのパーサが壊れる）と、ストリーミングが実質効かず全文一括で返る挙動が致命的
- **Kimi** — フラッグシップ k3 が sonnet と同額で乗り換える意味がなく、廉価版も会話が冗長という評判どおり。学習利用ポリシーも不明瞭で机上調査で見送り
- **MiniMax** — structured outputs 非対応で思考テキストが本文に混ざる仕様のため、フィードバック経路に載せるには追加実装が必要。品質評判も皆無で優先度を下げたまま終了

残っている節約チャンスは LLM ではなく**音声（Alibaba の TTS/STT、現行比 約 1/3）**で、別タスクとして `TODO.md` にある。
