...
| 3 | 読み上げ（TTS） | Gemini `gemini-3.1-flash-tts-preview`（2026-08-03 に Qwen から戻した） | 入力トークン + **音声出力トークン**（`usageMetadata`。25 トークン/秒） | AI 発話の **1 文ごと**に SSE リクエスト | 実装済み |
...
| Gemini 3.1 Flash TTS preview（TTS・既定。2026-08-03 に復帰） | 入力 $1 / 1M トークン、音声出力 $20 / 1M トークン（25 トークン/秒） | **生成音声 1 分 ≈ $0.03**（1,500 トークン） |
| （参考）Alibaba `qwen3-asr-flash-realtime`（STT 切替用。検証の結果見送り） | 音声 $0.000090 / 秒 | 1 分 ≈ $0.0054 |
| （参考）Alibaba `qwen3-tts-instruct-flash-realtime`（TTS 切替用。2026-08-01〜08-03 に既定だった） | $0.13 / 1 万文字（instruct の単価は未公表のため base の 2026-07-31 確認値を暫定計上） | 生成音声 1 分 ≈ $0.0098（Gemini の約 1/3） |
...
- 実装: `Voice/CloudPipeline/GeminiTTSClient.swift` + `CloudSentenceSpeaker.swift`（切替用に `QwenTTSClient.swift` / `OpenAITTSClient.swift`）
- Claude のストリーミングから文が切り出されるたびに **1 文 = 1 リクエスト**（SSE。Qwen と違い接続を張りっぱなしにはしない）
- 課金は入力トークン + **音声出力トークン**（$20 / 1M、25 トークン/秒）。`usageMetadata` から取る。
  実測換算で **生成音声 1 分 ≈ $0.03**
- **スタイル前置文は毎文の入力トークンに乗る**（Gemini は独立した instructions フィールドを持たず、
  本文先頭に自然文で前置する形のため）。ただし入力単価は $1 / 1M なので、支配的なのは音声出力側
- **barge-in 時の注意**: 先読みで取得済み・取得中だった未再生の文も生成された分は課金される（従来どおり）
- Qwen TTS へは `-tts-provider qwen` か `SentenceTTSClientFactory.Configuration.provider` の 1 箇所で切り替えられる（課金文字数 ≈ $0.0098/分）
- キャラの voice / スタイル前置文は `ChatCharacter.speechStyle`（Chobi=Leda / Naruko=Aoede）。Gemini は独立した instructions フィールドを持たないため本文先頭に自然文で前置する
...
| TTS（生成音声 5 分、Gemini 3.1 Flash） | 5 × $0.03 | $0.15 |
...
- 毎日 1 セッションで **月 $16 前後**が目安（Qwen TTS に切り替えると TTS が $0.05 になり 約 $0.32 / 月 $9.5 前後）
...
- 取得元: Claude は SSE の `message_start` / `message_delta`（非ストリーミングは応答の `usage`）、
  ...
  Qwen TTS（切替時）は `response.done` の `usage.characters`（課金文字数。+ 受信 PCM バイト数からの秒数）
...
1. **TTS が再び最大要因**（$0.15/セッション）。2026-08-01〜08-03 は Qwen で $0.05 まで下がっていた。コストを詰めるなら `-tts-provider qwen` で戻すのが最も効く
