...
| 出題ソース | **ユーザーが単語（熟語）を入力** / 前に練習した語のピル / **単語帳（esl.chobi.me）から選択**（2026-07-29 追加）。候補の AI 生成はしない |
...
| 覚えた単語の管理 | 出題した語をセッション履歴に残し、**次の単語カードにピルで並べて再練習できる**（2026-07-28 追加）。復習期日・重複回避の制御はしない |
...
| 記憶ノート | 注入・更新する | **しない** |
| カード | 生成候補 3 件 + 固定候補 + 🔄 / ＋ | 前に練習した語のピル + 入力 / 単語帳ボタン |
...
| カード本体 | 候補 3 件 + 固定候補「話しかける」+ 🔄 / ＋ | 前に練習した語のピル + 「単語を入力」「単語帳から選ぶ」ボタン |
...
- **単語カード**: 候補は生成しない。未使用のカードには**前に練習した語**をピルで並べ（新しい順・
  重複除去済み・最大 `ChatRoomStore.wordSuggestionCount` = 6 件。`LazyVGrid` で折り返す）、
...
- **単語帳から選ぶ**（2026-07-29 追加）: カードの「単語帳から選ぶ」ボタンで、隣のプロジェクト
  esl-learning-assistant（https://esl.chobi.me）に登録済みの単語帳から選ぶシートを開く。
  読み取り専用 API `GET /api/words`（`X-API-Secret` 認証・シークレットは Keychain の
  `wordbook-api-secret`）で新しい順に一覧し、サーバ検索（`q`・300ms デバウンス）と
...
### 前に練習した語のピル（2026-07-28 追加）

- 単体テスト: `PracticedWordSuggestionsTests`（重複除去・新しい順・件数上限・空白の扱い・正規化キー）と
  `ChatHistoryStoreTests`（`recentWords` が新しい順）。XCTest 205 件パス
- シミュレータ E2E: `-practice-mode word -start-word "get around to" ... -end-session` で終了後の
  カードにピル `get around to` が出ることを確認。続けて `-practice-mode word -start-from-card` で
  そのピルからセッションが始まる（`session: 開始 topic=get around to practice=word`）ことを確認。
  `-start-from-card` は単語モードではピルの 1 件目をタップした扱いにしている
- **実機確認はユーザーが実施し OK**（2026-07-29。ピルの折り返し・タップ範囲・長い熟語の表示）

### 単語帳からの出題（2026-07-29 追加）

- 単体テスト: `WordBookClientTests`（リクエスト組み立て・熟語のエンコード・null 許容の
  デコード・エラー変換）と `WordBookPickerStoreTests`（初回 / 再試行・シークレット未設定の案内・
  検索デバウンスと世代ガード・追い読みの終端 / 重複除去 / 失敗リトライ）。XCTest 221 件パス
- シミュレータ E2E（起動引数 `-open-wordbook` / `-wordbook-query` / `-pick-first-wordbook-word` /
  `-wordbook-base-url` を追加）: ローカル backend（run-server.sh）に向けて
  一覧表示 → サーバ検索 → 行タップでシートが閉じて `単語: book` のセッション開始、
  401（不正シークレット）とシークレット未設定の案内 + 再試行、まで確認
- 本番（esl.chobi.me）: 実装当日は `GET /api/words` 追加コミットが未デプロイで 404 だったが、
  同日中に esl-learning-assistant 側をデプロイして解消。`curl` で HTTP 200（total=185）を確認し、
  シミュレータでも本番 API に対する一覧表示を確認した。**実機確認はユーザーが実施し OK**（同日）

- 単語候補の自動生成（フィードバックの `try_phrases` からの出題）
  ※「マイ単語帳」からの出題は 2026-07-29 に単語帳（esl.chobi.me）連携として追加した（画面節参照）
- 復習スケジュール（SRS）・出題済み語の重複回避・定着判定（単語帳側の `reviewState` も使わない）
