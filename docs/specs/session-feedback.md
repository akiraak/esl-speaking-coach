...
セッション終了後に会話全文を `claude-sonnet-5` で評価し、テキストベースのフィードバックカードを
タイムラインへ投稿する（2026-07-31 に `claude-opus-5` から変更。単発で最も高額な呼び出しだったため）。
...
- モデル `claude-sonnet-5`（`SessionFeedbackClient.model` の 1 箇所で変更可） / **ストリーミング**（SSE のテキストデルタを蓄積し、終了後に JSON を
