...
struct OpenAITranscriptionConfiguration: Sendable {
    ...
    /// クライアント VAD + 手動 commit（サーバ VAD なし）で動くモデルか。
    /// 送信ゲート・入力の窓の制御はこのフラグで分岐する
    var usesClientEndpointing: Bool { isLiveTranscribe || isQwenASR }

    /// マイクから STT へ送る PCM のサンプルレート。Qwen3-ASR は公式ドキュメント上
    /// pcm 8/16kHz のみのため 16kHz へ落とす（リサンプルは AudioTapRouter.convert が担う）
    var micSampleRate: Double { isQwenASR ? 16000 : 24000 }
